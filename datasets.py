    ...
    def __getitem__(self, i):
        ...
        if self.split == 'TRAIN':
            ...
            for k in range((i // self.cpi) * self.cpi, ((i // self.cpi) * self.cpi) + self.cpi):
                ...
                caption_setlist1.append(one_caption.tolist())
            all_captions = torch.LongTensor(caption_setlist1)
            return ori_img, CPP, caption, mask, caplen
        else:
            caption_setlist = []
            for k in range((i // self.cpi) * self.cpi,((i // self.cpi) * self.cpi) + self.cpi):
                one_caption, _ = self.pad_tokens(k)
            ...
            return ori_img, CPP, caption, mask, caplen, all_captions
