...
from utils1 import get_eval_score
...
def evaluate(args, dataloader,tokenizer, model):
    ...
    CPP_correct_nochange = 0
    total = 0
    with torch.no_grad():
        for i, (ori_img, CPP, caps, mask, caplens, all_captions) in enumerate(tqdm(dataloader, desc=" EVALUATING AT BEAM SIZE " + str(args.beam_size))):
            # if i>20:
            #     break
            if (i + 1) % 5 != 0:
                continue
            ...
            Sim_cls_AB, pre_CPP, inputs_embeds, loss_de = model.Image_Encoder(CPP, ori_img)
            pred_CPP = torch.argmax(pre_CPP, 1)
            ...
            while True:
                ...
                if model.decoder_mode == 'gpt2':
                    ...
                    out = out.logits
                    logits, pre = model.dual_branch_func(pred_CPP, out)
                ...
                step += 1
            CPP_buff = CPP.clone()
            prediction = torch.argmax(pre_CPP, 1)
            correct += (prediction == CPP_buff).sum().float()
            total += len(CPP_buff)
            acc_str = (correct / total) * 100
            CPP_buff_nochange = CPP.clone()
            CPP_buff_nochange[CPP_buff_nochange > 0.5] = 2
            CPP_correct_nochange += (prediction == CPP_buff_nochange).sum().float()
            CPP_acc_nochange = (CPP_correct_nochange / total) * 100
        ...
        print('nochange_acc_nochange:', CPP_acc_nochange * 2)
        print('change_acc:', acc_str * 2 - CPP_acc_nochange * 2)
        return metrics
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default="" + dataset_name,
                        help='folder with data files saved by create_input_files.py.')
    parser.add_argument('--data_name', default=dataset_name + "_5_cap_per_img", help='base name shared by data files.')

    parser.add_argument('--model_path', default='')#./checkpoints/train_1_method_10/3-times/
