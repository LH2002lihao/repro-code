...
from decouple import DecoupleModule
from loadmodel import Pretrained_model
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, input, dif):
        ...
        output = self.cross(input, dif)  # (Q,K,V)
...
class CrossPromptFusion(nn.Module):
    def __init__(self):
        super().__init__()
        self.cross_attn = nn.MultiheadAttention(768, num_heads=4).to(device)

    def forward(self, prompt_A, prompt_B):
        prompt_A.to(device)
        prompt_B.to(device)
        # prompt_A: [3,16,768], prompt_B: [2,16,768]
        fused_A, _ = self.cross_attn(
            prompt_A.transpose(0,1),
            prompt_B.transpose(0,1),
            prompt_B.transpose(0,1)
        )
        return fused_A.transpose(0,1).to(device)  # [16,3,768]
...
class Image_Encoder(nn.Module):
    ...
    def __init__(self, clip_model_type, len_change_emmbed, clip_feat_dim, h=7, w=7, gpt_dim=768, n_head=8, n_layers=3, prompt_len=10, uni_prompt_1_len=0):
        ...
        # cls_model
        self.CPPM = Pretrained_model(decoder_mode='CPPM', finetune_gpt2=False,
                                         img_feature_h=7,
                                         img_feature_w=7)
        ...
        self.CPPM.load_state_dict(model)
        self.CPPM.eval()
        self.decouple = DecoupleModule().to(device)
        self.decouple.train()
    ...
    def changeflag2prompt(self, CPP):
        batch = CPP.shape[0]
        changefilter = CPP.unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, CPP, ori_img):
        ...
        change_proto_prompt = self.changeflag2prompt(CPP)
...
class LEVIR_CC_CaptionModel(nn.Module):
    def __init__(self, encoder_mode, decoder_mode, prompt_len, uni_prompt_1_len, len_change_emmbed,
        ...
        if self.decoder_mode == 'gpt2':
            gpt2_type = ''
            self.gpt_decoder = GPT2LMHeadModel.from_pretrained(gpt2_type)
    ...
    def dual_branch_func(self, CPP, out):
        output = self.lm_head_change(out)
        return output, 0

    def forward(self, tokens, changeflag, ori_img, mask: Optional[torch.Tensor] = None):

        Sim_cls_AB, pre_flag, prefix_projections, loss_de = self.Image_Encoder(changeflag, ori_img)

        embedding_text = self.gpt_decoder.transformer.wte(tokens)
        loss = 0
        ...
        return loss, pre_flag, output, loss_de
