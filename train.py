...
from utils1 import *
from save_log import  save_training_log
from CC import LEVIR_CC_CaptionModel
from eval12 import evaluate
...
setproctitle.setproctitle("lihao")
...
def train(args, train_loader, Caption_model, Caption_model_optimizer,Caption_model_lr_scheduler, epoch):
    ...
    total = 0
    accum_steps = 3
    for idx, (ori_img, CPP, caps, mask, caplens) in enumerate(train_loader):
        # if idx ==10:
        #     break
        ...
        # Forward prop.
        if args.dataset_name == 'LEVIR_CC':
            # print("caps", caps.shape)  # caps torch.Size([1, 50])
            # print("mask", mask.shape)  # mask torch.Size([1, 154])
            Sim_cls_AB, pre_flag, outputs, loss_de = Caption_model(caps, CPP, ori_img, mask)
        ...
        # pre_flag_loss
        CPP_buff = CPP.clone()
        pre_flag_loss = F.cross_entropy(pre_flag, CPP_buff)  # , ignore_index=0
        # print("loss_cap", loss_cap)
        # print("loss_de", loss_de)
        # total loss
        loss = loss_cap + 3 * loss_de
        # loss = loss_cap
        ...
        correct += (prediction == CPP_buff).sum().float()
        total += len(CPP_buff)
...
def main(args):
    ...
    train_loader = torch.utils.data.DataLoader(
        CaptionDataset(args.clip_model_type, args.dataset_name, args.data_folder, args.data_name, 'TRAIN',args.prefix_length),
        batch_size=1, shuffle=True, num_workers=args.workers, pin_memory=True, persistent_workers=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epochs_since_improvement > 0 and epochs_since_improvement % 10 == 0:
            adjust_learning_rate(Caption_model_optimizer, 0.8)
        ...
        save_training_log(
            args=args,
            epoch=epoch,
            batch_info={"current": 0, "total": len(test_loader)},  # 验证阶段标记为batch 0
            metrics=metrics,
            phase='val'
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default=""+dataset_name,help='folder with data files saved by create_input_files.py.')
    ...
    parser.add_argument('--stop_criteria', type=int, default=30, help='training stop if epochs_since_improvement == stop_criteria')
    ...
    parser.add_argument('--model_lr', type=float, default=1e-5, help='learning rate for encoder if fine-tuning.')
